...
InternalHandoffRequestList::InternalHandoffRequestList(std::shared_ptr<iop::Component> cmp, unsigned char request_timeout, unsigned char decision_timeout)
: logger(cmp->get_logger().get_child("InternalHandoffRequestList"))
{
	this->decision_timeout = decision_timeout;
...
std::shared_ptr<iop::HandoffRequest> InternalHandoffRequestList::update(JausAddress requestor, int64_t current_ts)
{
	...
	for (it = p_requests.begin(); it != p_requests.end(); ++it) {
		if (requestor == it->get()->requestor) {
			it->get()->ts_request = p_resolve_ts(current_ts);
			return *it;
		}
	}
	std::shared_ptr<HandoffRequest> nullPtr;
...
std::shared_ptr<iop::HandoffRequest> InternalHandoffRequestList::get_first_expired_request(int64_t current_ts)
{
	lock_type lock(p_mutex);
	std::shared_ptr<HandoffRequest> result;
	std::vector<std::shared_ptr<HandoffRequest> >::iterator it;
	for (it = p_requests.begin(); it != p_requests.end(); ++it) {
		if (expired_request(it->get()->ts_request, current_ts)) {
			result = *it;
			return result;
		}
	}
...
bool InternalHandoffRequestList::expired_decision_request(int64_t current_ts)
{
	if (ts_request > 0) {
		return p_resolve_ts(current_ts) > ts_request + decision_timeout;
	}
	return false;
}

bool InternalHandoffRequestList::expired_request(int64_t ts_request, int64_t current_ts)
{
	if (ts_request > 0) {
		RCLCPP_DEBUG(logger, "COMPARE ts %d, %d, timeout: %d", (int)ts_request, (int)p_resolve_ts(current_ts), request_timeout);
		return p_resolve_ts(current_ts) > ts_request + request_timeout;
	}
	return false;
}

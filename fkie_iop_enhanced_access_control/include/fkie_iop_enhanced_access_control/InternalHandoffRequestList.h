...
public:
	...
	HandoffRequest(unsigned char id, JausAddress requestor, unsigned char authority, std::string explanation) {
		...
		ts_request = iop::Component::now_secs();
	}
...
public:
	InternalHandoffRequestList(std::shared_ptr<iop::Component> cmp, unsigned char request_timeout=10, unsigned char decision_timeout=60);
	...
	std::shared_ptr<iop::HandoffRequest> get_first_expired_request(int64_t current_ts=0);
	/** Returns a copy of all HandofRequests. */
	std::vector<HandoffRequest> get_all();

	/** Tests if given `ts_request` has expired.
	 * :param current_ts: current time stamp in seconds. If 0 the time is determine by this method.
	 * :type current_ts: int64_t */
	bool expired_decision_request(int64_t current_ts=0);

	/** Tests if given `ts_request` has expired.
	 * :param ts_request: time stamp of last request in seconds. Returns `false` on 0.
	 * :type ts_request: int64_t
	 * :param current_ts: current time stamp in seconds. If 0 the time is determine by this method.
	 * :type current_ts: int64_t */
	bool expired_request(int64_t ts_request, int64_t current_ts=0);
	...
	unsigned char decision_timeout;
	...
	unsigned char request_timeout;

	/// time stamp in seconds of last RequestReleaseControl request. No requests are defined by 0.
	int64_t ts_request;

...
EnhancedAccessControl_ReceiveFSM::EnhancedAccessControl_ReceiveFSM(urn_jaus_jss_core_Transport::Transport_ReceiveFSM* pTransport_ReceiveFSM, urn_jaus_jss_core_Events::Events_ReceiveFSM* pEvents_ReceiveFSM, urn_jaus_jss_core_AccessControl::AccessControl_ReceiveFSM* pAccessControl_ReceiveFSM)
{
...
void EnhancedAccessControl_ReceiveFSM::setupNotifications()
{
	...
	iop::Config cfg("~EnhancedAccessControl");
	...
	p_timeout_timer = p_nh.createTimer(ros::Duration(1.0), &EnhancedAccessControl_ReceiveFSM::p_timeout, this);
}
...
void EnhancedAccessControl_ReceiveFSM::processHandoffResponseAction(ConfirmReleaseControl msg, Receive::Body::ReceiveRec transportData)
{
	...
	for (unsigned int i = 0;i < msg.getBody()->getReleaseControlList()->getNumberOfElements(); i++) {
		...
		boost::shared_ptr<iop::HandoffRequest> requester = p_requests.get(id);
		ROS_DEBUG_NAMED("EnhancedAccessControl", "handoff response for id %d, code %s from %s", (int)id, p_crc_code2str(code).c_str(), address.str().c_str());
		if (requester) {
			...
			ROS_WARN_NAMED("EnhancedAccessControl", "request for handoff response with id %d from %s not found", (int)id, address.str().c_str());
		}
	}
}
...
void EnhancedAccessControl_ReceiveFSM::queueHandoffRequestAction(RequestHandoff msg, Receive::Body::ReceiveRec transportData)
{
	...
	ROS_DEBUG_NAMED("EnhancedAccessControl", "queue handoff request from %s, authority %d, explanation %s", address.str().c_str(), authority, explanation.c_str());
	boost::shared_ptr<iop::HandoffRequest> added = p_requests.add(address, authority, explanation);
	if (added) {
		added->ts_enhanced_request = ros::Time::now().sec;
		...
		// Send a ConfirmHandoffRequest message to querying client
		ROS_WARN_NAMED("EnhancedAccessControl", "queue handoff request from %s, authority %d, explanation `%s` failed!", address.str().c_str(), authority, explanation.c_str());
		ConfirmHandoffRequest reply;
...
void EnhancedAccessControl_ReceiveFSM::removeHandoffRequestAction(RemoveHandoffRequest msg, Receive::Body::ReceiveRec transportData)
{
	...
	ROS_DEBUG_NAMED("EnhancedAccessControl", "remove handoff request by id %d from %s", id, address.str().c_str());
	boost::shared_ptr<iop::HandoffRequest> removed = p_requests.remove(id);
...
{
	JausAddress address = transportData.getAddress();
	ROS_DEBUG_NAMED("EnhancedAccessControl", "remove handoff request by address %s", address.str().c_str());
	boost::shared_ptr<iop::HandoffRequest> removed = p_requests.remove(address);
	// inform the controller about queue changes
	if (removed) {
		p_send_request_release_control();
	}
...
void EnhancedAccessControl_ReceiveFSM::sendConfirmHandoffRequestAction(std::string arg0, Receive::Body::ReceiveRec transportData)
{
	...
	ROS_DEBUG_NAMED("EnhancedAccessControl", "send ConfirmHandoffRequest %s[%d] to %s", arg0.c_str(), code, address.str().c_str());
...
{
	JausAddress address = transportData.getAddress();
	ROS_DEBUG_NAMED("EnhancedAccessControl", "report enhanced timeout %d sec to %s", (int)p_requests.enhanced_timeout, address.str().c_str());
...
{
	JausAddress address = transportData.getAddress();
	ROS_DEBUG_NAMED("EnhancedAccessControl", "report handoff timeout %d sec to %s", (int)p_requests.handoff_timeout, address.str().c_str());
...
void EnhancedAccessControl_ReceiveFSM::updateHandoffRequestAction(RequestHandoff msg, Receive::Body::ReceiveRec transportData)
{
	...
	unsigned char authority = msg.getBody()->getRequestHandoffRec()->getAuthorityCode();
	std::string explanation = msg.getBody()->getRequestHandoffRec()->getExplanation();
	ROS_DEBUG_NAMED("EnhancedAccessControl", "update handoff request from %s, authority: %d, explanation: %s", address.str().c_str(), (int)authority, explanation.c_str());
...
void EnhancedAccessControl_ReceiveFSM::p_timeout(const ros::TimerEvent& event)
{
	ros::Time now = ros::Time::now();
	boost::shared_ptr<iop::HandoffRequest> enh_req = p_requests.get_first_expired_enhanced_request(now.sec);
	if (enh_req) {
		ROS_WARN_NAMED("EnhancedAccessControl", "update for request handoff from ocu %s expired, remove request!", enh_req->requestor.str().c_str());
	...
	if (p_requests.expired_handoff_request(now.sec)) {
		p_requests.ts_handoff_request = 0;
		std::vector<iop::HandoffRequest> requests = p_requests.get_all();
		std::vector<iop::HandoffRequest>::iterator it;
		for (it = requests.begin(); it != requests.end(); ++it) {
			ROS_WARN_NAMED("EnhancedAccessControl", "request handoff to controlled ocu %s expired, inform the requested OCU: %s!",
...
bool EnhancedAccessControl_ReceiveFSM::p_send_request_release_control()
{
	...
	for (it = requests.begin(); it != requests.end(); ++it) {
		ROS_DEBUG_NAMED("EnhancedAccessControl", "send request release control to controlled ocu %s for requested OCU: %s, authority: %d, explanation: `%s`",
	...
	if (p_requests.ts_handoff_request == 0) {
		p_requests.ts_handoff_request = ros::Time::now().sec;
	}

...
void HandoffController_ReceiveFSM::setupIopConfiguration()
{
    ...
    if (p_auto_request && p_request_timeout > 0) {
        p_timer.set_rate(p_request_timeout / 3.0);
    } else {
        p_timer.set_rate(10);
    }
}
...
void HandoffController_ReceiveFSM::processConfirmHandoffRequestAction(ConfirmHandoffRequest msg, Receive::Body::ReceiveRec transportData)
{
    ...
    unsigned char id = msg.getBody()->getConfirmHandoffRequestRec()->getRequestID();
...
void HandoffController_ReceiveFSM::processHandoffRequestAction(RequestReleaseControl msg, Receive::Body::ReceiveRec transportData)
{
    JausAddress sender = transportData.getAddress();
    std::vector<JausAddress> current_list;
    RequestReleaseControl::Body::RequestReleaseControlRec* rr_rec = msg.getBody()->getRequestReleaseControlRec();
...
void HandoffController_ReceiveFSM::processReportHandoffDecisionTimeoutAction(ReportHandoffDecisionTimeout msg, Receive::Body::ReceiveRec transportData)
{
    /// Insert User Code HERE
}

void HandoffController_ReceiveFSM::processReportHandoffRequestTimeoutAction(ReportHandoffRequestTimeout msg, Receive::Body::ReceiveRec transportData)
{
    unsigned char timeout = msg.getBody()->getReportHandoffRequestTimeoutRec()->getHandoffRequestTimeout();
    if (p_request_timeout > timeout) {
        JausAddress sender = transportData.getAddress();
        RCLCPP_DEBUG(logger, "update EnhancedTimeout to %d from %s", timeout, sender.str().c_str());
        p_request_timeout = timeout;
        // restart timer, if already running
        p_timer.stop();
        if (p_request_timeout != 0) {
            RCLCPP_DEBUG(logger, "create timer with new period %.2f", p_request_timeout / 3.0);
            p_timer.set_rate(p_request_timeout / 3.0);
            p_timer.start();
        }
    }
...
void HandoffController_ReceiveFSM::removeRequestAndSendConfirmReleaseControlAction(RemoveRequestReleaseControl msg, Receive::Body::ReceiveRec transportData)
{
    JausAddress sender = transportData.getAddress();
    ConfirmReleaseControl reply;
    ConfirmReleaseControl::Body::ReleaseControlRec rcrec;
    auto id = msg.getBody()->getRemoveRequestReleaseControlRec()->getID();
    reply.getBody()->getReleaseControlRec()->setID(id);
    reply.getBody()->getReleaseControlRec()->setResponseCode(3); // REMOVED
    RCLCPP_DEBUG(logger, "RemoveRequestReleaseControl to %s, id: %d", sender.str().c_str(), id);
...
{
    ConfirmReleaseControl reply;
    ConfirmReleaseControl::Body::ReleaseControlRec* rcrec = reply.getBody()->getReleaseControlRec();

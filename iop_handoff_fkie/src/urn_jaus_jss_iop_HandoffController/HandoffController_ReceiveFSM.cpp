...
void HandoffController_ReceiveFSM::setupNotifications()
{
	...
	p_pub_handoff_response = cfg.advertise<iop_msgs_fkie::HandoffResponse>("handoff_remote_response", 5);
	p_pub_handoff_request = cfg.advertise<iop_msgs_fkie::HandoffRequest>("handoff_remote_request", 5);
	p_sub_handoff_request = cfg.subscribe("handoff_own_request", 5, &HandoffController_ReceiveFSM::p_ros_handoff_request, this);
	p_sub_handoff_response = cfg.subscribe("handoff_own_response", 5, &HandoffController_ReceiveFSM::p_ros_handoff_response, this);
...
void HandoffController_ReceiveFSM::processConfirmHandoffRequestAction(ConfirmHandoffRequest msg, Receive::Body::ReceiveRec transportData)
{
	...
	if (remove) {
		...
		std::vector<JausAddress>::iterator it = std::find(p_in_request.begin(), p_in_request.end(), sender);
		if (it != p_in_request.end()) {
			p_in_request.erase(it);
		}
		std::vector<JausAddress>::iterator it2 = std::find(p_rejected_addresses.begin(), p_rejected_addresses.end(), sender);
		if (it2 != p_rejected_addresses.end()) {
			p_rejected_addresses.erase(it2);
		}
	}
}
...
void HandoffController_ReceiveFSM::p_accesscontrolclient_reply_handler(JausAddress &address, unsigned char code)
{
	...
	if (code == AccessControlClient_ReceiveFSM::ACCESS_STATE_INSUFFICIENT_AUTHORITY) {
		...
		if (std::find(p_rejected_addresses.begin(), p_rejected_addresses.end(), address) == p_rejected_addresses.end()) {
			...
			if (p_auto_request) {
				p_in_request.push_back(address);
			}
		}
	} else {
		std::vector<JausAddress>::iterator it = std::find(p_rejected_addresses.begin(), p_rejected_addresses.end(), address);
		if (it != p_rejected_addresses.end()) {
			p_rejected_addresses.erase(it);
			if (p_auto_request) {
				// remove from requests
				std::vector<JausAddress>::iterator it2 = std::find(p_in_request.begin(), p_in_request.end(), address);
				if (it2 != p_in_request.end()) {
					p_in_request.erase(it);
				}
			}
		}
...
void HandoffController_ReceiveFSM::p_ros_handoff_request(const iop_msgs_fkie::HandoffRequest::ConstPtr msg)
{
	...
	if (msg->request) {
		...
		p_in_request = p_rejected_addresses;
